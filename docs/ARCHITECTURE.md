...
`llm/config.py` and `config/llm.deepseek_v1.json` form a strict configuration-only boundary. They are not injected into `composition.py`, create no provider client, and do not alter the deterministic Policy/router topology.
...
Candidate selection removes one ID from the authoritative queue. Surrogate admission evaluates the same configured rules and compares a hard-first/soft-second violation rank against the current Best; it rejects new hard failures and no longer consumes `screening_score` as control evidence. Screen rejection or valid non-PASS HFSS evidence consumes the current candidate and returns for another decision. When the queue is empty, Policy may rebuild intent/objective from the latest evaluated candidate diagnosis, falling back to baseline evidence when only surrogate-rejected candidates were consumed. All hard+soft rules produce full success; hard PASS with remaining soft failures enters margin expansion and, if a bound expires, finalizes with `core_pass_margin_incomplete` rather than an unqualified target-met claim.
...
`OptimizerRequest` is the only formal optimizer input. It binds Run/context, baseline parameters and the complete surrogate response, `DesignGoal`, diagnosis digest, target specification, Agent `OptimizationObjective`, its translated `EffectiveObjective`, and provider/config fingerprints. The worker derives the authoritative optimizer frequency grid from that baseline response; Production therefore uses exactly the HFSS contract's 0.1–20 GHz/200 samples. It materializes one runtime objective per configured rule and uses the Production-only geometry/formula/passivity constraint set.

Surrogate reranking is control evidence, not an ephemeral local sort. `ObjectiveRank` is lexicographic: invalid, hard-failure count, maximum hard violation, total hard violation, soft-failure count, total soft violation, then stable per-rule violations. Each candidate's canonical surrogate result, EvaluationResult, immutable receipt identity/SHA, rank, and request/objective digests are persisted and replayed.
...
| DeepSeek LLM provider configuration | CODE PRESENT / STRICT JSON | UNIT TESTED; provider call NOT RUN | Configuration only; not reachable from any workflow |
...
| Optimization-outcome A/B diagnostic | WIRED / DEFAULT-DISABLED / NON-CANARY | REAL HFSS VERIFIED for exact frozen Baseline/P0028 pair; local improvement observed | Separate bounded physical experiment; never authorizes Production |
...
The graph constructs one canonical `OptimizerRequest` from manifest goal, diagnosis/surrogate evidence, Agent objective, and provider/config fingerprints. `OptimizationObjectiveBuilder` emits every evaluation rule with identity, parameter, operator, threshold, band, hard/soft flag, signed margin, and violation. `EffectiveObjective` maps `<=` to `max(0, band_max-target)` and `>=` to `max(0, target-band_min)`, always minimizing exact dB violation. The worker passes the request-bound 200-point grid and `constraints.production_agent_v1.csv` into vendor execution, verifies the echoed rows, and returns the full Pareto set.
...
`DeterministicEvaluator.evaluate_sparameters` evaluates explicit S11/S21 hard/soft band rules with one shared signed-margin function: nonnegative always passes and negative magnitude is the exact violation. WF-001 loads `production-evaluation-v1`: Core 6–18 GHz HARD `S21_dB <= -30 dB` and `S11_dB >= -0.5 dB`, with identical SOFT targets on 5–6 and 18–19 GHz. Every sampled point in each band is constrained; 0.1–5 and 19–20 GHz remain stored/observable but do not enter the performance objective. `EvaluationComparator`, ranking, gate, and Best promotion consume the same rule semantics.
...
Campaign `hfss-optimization-diagnostic-20260826-085032` exercised that path at exact revision `454c4f3...`. Both physical actions completed and the immutable report observed a small favorable aggregate S11 outcome, while also exposing a 12.3–20.0 GHz local reversal and materially overpredicted improvement. This verifies the diagnostic architecture and the frozen candidate's local outcome, not the equivalent model's global fidelity.
