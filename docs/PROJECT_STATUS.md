...
- **ISSUE-036 — OPEN / CURRENT REAL BLOCKER:** four campaign `batch.log` files prove FlexNet `-15,10` for `hfss_gui` at `1055@localhost`. Port 1055 is closed, the matching service is stopped, and the available local license provenance is not acceptable for use. A legitimate ANSYS/organization license server must be configured by an authorized administrator.
...
Obtain a legitimate reachable ANSYS/organization license server and update the administrator-controlled license configuration. Recheck the endpoint/entitlement without solving, then rerun the offline suite and issue a fresh clean-HEAD three-case campaign; never resume any failed Calibration Run. Only a passing immutable Calibration report permits Canary readiness and WF-001.
