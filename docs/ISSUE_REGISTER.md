...
- **Severity / status:** BLOCKER / RESOLVED OFFLINE on 2026-08-24; physical rerun pending
- **Trigger/evidence:** the third authorized probe started AEDT 2025 R1, created `pa_multi.aedt`, and attempted to insert only `interposer_temple4`. PyAEDT 0.18.1 `Desktop.active_design` assumed `SetActiveDesign` returned an object, but gRPC returned `None`/`bool`; its decorated `Hfss.__init__` consequently returned `False`, producing `TypeError`. A fourth exact-revision probe invoked the first compatibility hook and proved that one early activation plus read-only polling still failed for 30 seconds. Neither probe reached Builder geometry, setup, solve, or Touchstone.
- **Resolution:** the isolated worker installs a narrow compatibility hook before Hfss construction. For an explicitly requested design only, it retries `SetActiveDesign(name)` as well as `GetDesign(name)`/`GetActiveDesign()` for at most 30 seconds, records `GetTopDesignList()` in a terminal error, and requires exact `GetName()` equality. It never selects or creates a fallback design.
- **Safety/evidence:** both failed attempts were explicitly reconciled as confirmed failed with immutable evidence; the fourth is operation `op_ad9593d53eebacb86685fa638e62e6ff`, attempt `att_092ca07a34dc4a2166c18159ba67e20d`, evidence `art_f614467f305ca1930bbf235cd6725519`. No process/Agent lock remained; no new attempt/refund/retry occurred. Unit regressions prove bool acknowledgement, delayed repeated activation, and wrong-design fail-closed behavior.
