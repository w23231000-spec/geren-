"""Strict controller contracts for the opt-in offline closed-loop Agent."""
...
OFFLINE_CLOSED_LOOP_POLICY_ID = "bounded-offline-policy-v1"
...
class ClosedLoopControllerState:
    ...
    def initial(
        ...
        budget: ClosedLoopBudget,
        *,
        policy_id: str = OFFLINE_CLOSED_LOOP_POLICY_ID,
    ) -> "ClosedLoopControllerState":
        return cls(policy_id=policy_id, budget=budget)

    @classmethod
    def production_canary(cls) -> "ClosedLoopControllerState":
        return cls.initial(
            ClosedLoopBudget.production_canary(),
